...
# making a dataframe using the cifar-100 dataset
...
y_train = to_categorical(y_train, 100) #labels a category (ex:bird,plane,etc), 100 categories
...
num_classes = 100 #bc theres 100 categories
...
model_5 = tf.keras.models.Sequential( # 5 epochs: acc:42.4%, loss:222,9%, val_acc:41.3%, val_loss:231.0% || 10 epochs: acc:54.0%, loss:170.2%, val_acc:45.8%, val_loss:217.3%
    # definently overfit; more overfit after the full 10 than after 5, but slightly better accuracy after 10 compared to 5
    [
...
model_lol = tf.keras.models.Sequential( # 5 epochs: acc:16.3%, loss:383.3%, val_acc:13.3%, val_loss:421.7% || 10 epochs: acc:18.8%, loss:369.0%, val_acc:12.8%, val_loss:416.1%
    # it's amazing (lol)
    [
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(num_classes, activation='softmax')
...
# model summary
print('Model Summary:', model_lol.summary())